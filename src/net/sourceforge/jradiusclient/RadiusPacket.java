...
public class RadiusPacket {
	...
	/**
	 * RADIUS_HEADER_LENGTH is 20 bytes (corresponding to 1 byte for code + 1 byte
	 * for Identifier + 2 bytes for Length + 16 bytes for Request Authenticator) It
	 * is not a coincidence that it is the same as the MIN_PACKET_LENGTH
	 **/
	 ...
	 * @throws InvalidParameterException if the attributeList is null or contains
	 *                                   non-RadiusAttribute type entries
	 */
	 ...
	 *
	 * @param type          int a PacketType to send.
	 * @param attributeList a list of RadiusAttribute objects to initialize this
	 *                      RadiusPacket with
	 * @throws InvalidParameterException if the attributeList is null or contains
	 *                                   non-RadiusAttribute type entries
	 */
	 ...
	 * @throws InvalidParameterException
	 */
	public RadiusPacket(final int type, final byte identifier, final List<?> attributeList)
			throws InvalidParameterException {
		if ((type < 1) || (type > 256)) {
	 ...
	 * 
	 * @param attributeList a list of RadiusAttribute objects to add to this
	 *                      RadiusPacket
	 * @throws InvalidParameterException if the attributeList is null or contains
	 *                                   non-RadiusAttribute type entries
	 */
	...
	// do nothing method, sub-classes should implement to get attributes validation
	// during
	// the setAttribute and setAttributes methods
	protected void validateAttribute(final RadiusAttribute attribute) throws InvalidParameterException {
	 ...
	 * 
	 * @param attributeType an integer between 0 and 256 (i.e. a byte) from the list
	 *                      of Radius constants in
	 *                      net.sourceforge.jradiusclient.RadiusValues
	 * @return a single RadiusAttribute from the RadiusPacket
	 * @throws RadiusException           if no attribute of type attributeType is
	 *                                   stored in this RadiusPacket
	 * @throws InvalidParameterException if the attributeType is not between 0 and
	 *                                   256 (i.e. a byte)
	 */
	public RadiusAttribute getAttribute(int attributeType) throws InvalidParameterException, RadiusException {
		...
		synchronized (this.attributes) {
			tempRa = (RadiusAttribute) this.attributes.get(Integer.valueOf(attributeType));
		}
		if (null == tempRa) {
	...
	/**
	 * determine if this RadiusPacket contains at least one of this attribute type
	 * 
	...
	public boolean hasAttribute(int attributeType) {
		...
		synchronized (this.attributes) {
			hasAttribute = this.attributes.containsKey(Integer.valueOf(attributeType));
		}
		return hasAttribute;
	}
	...
	/**
	 * Return the packetIdentifier for this RadiusPacket. This can be used to match
	 * request packets to response packets
	 * 
	 ...
	 * @throws RadiusException If there is any error assembling the bytes into a
	 *                         byte array
	 */
